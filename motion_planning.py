...
class MotionPlanning(Drone):
    ...
    def local_position_callback(self):
        if self.flight_state == States.TAKEOFF:
            ...
            if -1.0 * self.local_position[2] > 0.9 * self.target_position[2]:
                self.waypoint_transition()
        elif self.flight_state == States.WAYPOINT:
            m = np.linalg.norm(self.local_velocity[0:2])
            m = np.clip(m,1,10)
 
            if np.linalg.norm(self.target_position[0:2] - self.local_position[0:2]) < 1.0*m:
    ...
    def takeoff_transition(self):
        ...
        print("takeoff transition", self.target_position[2], self.local_position[2])
